...
Current version: Rule-based interactive prototype used to validate the coaching workflow.
Future direction: LLM-generated coaching feedback and Agent / Tool Use architecture.
...
將目前的規則式回饋升級為 LLM 生成回饋，讓教練建議更自然，也更能理解使用者回覆背後的語意與情境。
...
AI 客戶會根據業務每一輪回覆即時反應，讓練習更接近真實電訪情境。

v0.6：Agent / Tool Use Architecture
將系統拆成多個工具模組，例如：

Customer Simulator：根據情境扮演不同類型客戶
Question Checker：檢查業務是否問到關鍵問題
Score Calculator：根據評分規則計算訓練分數
Feedback Generator：產生教練回饋與建議改寫
Report Renderer：輸出結構化訓練報告

這個版本會讓作品更接近 Agent / Tool Use 架構，而不只是單一頁面的規則式工具。

v1.0：Sales Training Dashboard

提供主管或 Sales Ops 查看多位業務的訓練紀錄、平均漏問問題、常見弱點與改善趨勢，協助團隊進行訓練追蹤與流程優化。
---
12. Resume Bullet
中文履歷版
